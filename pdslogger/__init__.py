...
class PdsLogger(logging.Logger):
    ...
    def __init__(self, logname, *, default_prefix='pds.', levels={}, limits={}, roots=[],
        ...
        full_logname = self._full_logname(logname, default_prefix)
        if self._LOGGER_IS_FAKE:
            self._logger = None
            self._logname = full_logname
            handlers = []
        else:
            if full_logname in _LOOKUP:
                raise ValueError(f'PdsLogger {full_logname} already exists')
            if logname in _LOOKUP:
                raise ValueError(f'PdsLogger {logname} already exists')
            if logname in logging.Logger.manager.loggerDict:    # Name found w/o prefix
                full_logname = logname

            _LOOKUP[full_logname] = self                        # Save logger in cache
            self._logname = full_logname
    ...
    def _merge_level_names(self, levels):
        ...
        for level_name, level_num in levels.items():
            ...
            if level_num < 0:
                ...
                # Update alias lookup...
                for name, alias in self._level_name_aliases.items():
    ...
    def set_limit(self, name, limit):
        ...
        self._limits_by_name[-1][self._repair_level_name(name)] = limit
    ...
    add_handlers = add_handler      # useful alias
